...
public class DrawingPhaseController : MonoBehaviour
{
    ...
    private bool IsMouseCloseToGuideLine(out bool isInsideGuideRect)
    {
        ...
        Canvas canvas = guideImage.canvas;
    ...
    else if (cameraToUse == null && canvas != null)
    {
        cameraToUse = canvas.worldCamera;
    }

    if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(
            guideRect, Input.mousePosition, cameraToUse, out Vector2 localPoint))
        return false;
...
private void AddUILinePointAtMouse()
{
    ...
    if (RectTransformUtility.ScreenPointToLocalPointInRectangle(
            lineRect, Input.mousePosition, cameraToUse, out Vector2 localPoint))
    {
